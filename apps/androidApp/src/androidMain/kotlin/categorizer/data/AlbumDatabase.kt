...
internal fun AlbumEntry.toValues(): ContentValues = ContentValues(16).apply {
    ...
    put("make", confirmedIdentity.scientificName.orEmpty().substringBefore(' '))
    put("model", confirmedIdentity.scientificName.orEmpty().substringAfter(' ', confirmedIdentity.scientificName.orEmpty()))
    put("generation_label", confirmedIdentity.attributes["generation_label"])
    put("approximate_year_range", confirmedIdentity.attributes["approximate_year_range"])
